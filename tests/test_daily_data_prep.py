...
@patch("sandboxes.data.knowledge_store.ingest_announcement", return_value=1)
@patch("sandboxes.data.cninfo_client.fetch_announcement_text", return_value="")
@patch("sandboxes.data.cninfo_client.query_all_announcements", return_value=[])
@patch("harness.daily_data_prep.akshare_client")
@patch("harness.daily_data_prep.tushare_client")
def test_run_daily_prep(mock_ts, mock_ak, mock_query_all, mock_fetch_text, mock_ingest):
    """Mock 所有 tushare/akshare 调用，验证 stats 返回各表行数。"""
    mock_ts.get_research_report.return_value = {
        "status": "ok",
        "data": [{"title": "报告A", "trade_date": "20260506", "abstr": "摘要"}],
    }
    mock_ts.get_npr.return_value = {
        "status": "ok",
        "data": [{"title": "政策A", "pubtime": "20260501", "puborg": "国务院"}],
    }
    mock_ts.get_hm_list.return_value = {
        "status": "ok",
        "data": [{"name": "赵老哥", "desc": "打板", "orgs": "华泰证券"}],
    }
